<script setup>
import { kk } from "../stores/kk.js";
import { Ad } from "../stores/lj.js";
const { Yam } = kk();
const { AP, Puss, ST } = Ad();
</script>
<template>
  ...
  <div class="row mt-3" v-for="n in Yam.length">
    <div class="col" v-if="Yam && Yam[n - 1]">{{ Yam[n - 1].Code }}</div>
    <div class="col" v-if="Yam && Yam[n - 1]">{{ Yam[n - 1].Name }}</div>
    <div class="col" v-if="Yam && Yam[n - 1]">{{ Yam[n - 1].group }}</div>
    <div class="col">
      <button
        class="btn btn-success"
        @click="Puss(Yam[n - 1].Code, Yam[n - 1].Name, Yam[n - 1].group)"
      >
        <svg
  ...
  <div class="row mt-3" v-for="n in AP.length">
    <div class="col">{{ n }}</div>
    <div class="col" v-if="AP && AP[n - 1]">{{ AP[n - 1].code }}</div>
    <div class="col" v-if="AP && AP[n - 1]">{{ AP[n - 1].name }}</div>
    <div class="col" v-if="AP && AP[n - 1]">{{ AP[n - 1].group }}</div>
    <div class="col">
      <button class="btn btn-success" @click="ST(n - 1)">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
